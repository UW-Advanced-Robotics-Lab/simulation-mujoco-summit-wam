...
<mujoco model="playground_wagon_manipulation(v1.0.0)">
    ...
    <actuator>
        ...
        <motor name='wam/J1/F' ctrllimited="true" joint='wam/J1' ctrlrange='-12.62 12.62' gear="42"      />
        <motor name='wam/J2/F' ctrllimited="true" joint='wam/J2' ctrlrange='-12.62 12.62' gear="28.25"   />
    </actuator>
...
# Time-step
delta_t = 0.002 # ms
# Keep track of current error in position
curr_error = np.array([[0],[0],[0],[0],[0],[0],[0]])
# Keep track of current change-in-error in position
curr_delta_error = np.array([[0],[0],[0],[0],[0],[0],[0]])
# Keep track of previous error in position
prev_error = np.array([[0],[0],[0],[0],[0],[0],[0]])
# Keep track of cummulative error in position
curr_cumu_error = np.array([[0],[0],[0],[0],[0],[0],[0]])
# Set-point
set_point = np.array([[0.613002679893322],
                      [-1.82125471189637*0],
                      [1.64210162647343*0],
                      [1.84239963058323*0],
                      [-2.88138686812642*0],
...
with mujoco.Renderer(model) as renderer:
  while data.time < duration:
    ...
    # Current-errors
    curr_error[0,0] = set_point[0,0]-data.joint('wam/J1').qpos[0]
    print(curr_error[0,0])
    curr_error[1,0] = set_point[1,0]-data.joint('wam/J2').qpos[0]
    # Current delta-errors
    curr_delta_error[0,0] = (curr_error[0,0]-prev_error[0,0])/delta_t
    curr_delta_error[1,0] = (curr_error[1,0]-prev_error[1,0])/delta_t
    # Current cummulative-error
    curr_cumu_error[0,0] = curr_cumu_error[0,0]+curr_error[0,0]*delta_t
    curr_cumu_error[1,0] = curr_cumu_error[1,0]+curr_error[1,0]*delta_t
    # Position control
    data.actuator('wam/J1/F').ctrl = 900*curr_error[0,0]+10*curr_delta_error[0,0]+2.5*curr_cumu_error[0,0]
    data.actuator('wam/J2/F').ctrl = 2500*curr_error[1,0]+20*curr_delta_error[1,0]+5*curr_cumu_error[1,0]
    # Replace previous-error with new error
...
print('wam/shoulder_yaw_link/J1')
print("Position: "+str(data.joint('wam/J1').qpos[0]))
print("Effort: "+str(data.joint('wam/J1').qfrc_actuator[0]))
print('wam/shoulder_pitch_link/J1')
print("Position: "+str(data.joint('wam/J2').qpos[0]))
print("Effort: "+str(data.joint('wam/J2').qfrc_actuator[0]))
